  ...
  <div class="pay">
    ...
    <div class="address">
      ...
      <div class="info" v-if="addressList.length > 0">
        <div class="info-content">
          <span class="name">{{ addressList[0].name }}</span>
          <span class="mobile">{{ addressList[0].phone }}</span>
        </div>
        <div class="info-address">
          {{ addressList[0].region.province }} {{ addressList[0].region.city }}
          {{ addressList[0].region.region }} {{ addressList[0].detail }}
        </div>
      </div>
    ...
    <div class="pay-list" v-if="order.goodsList">
      ...
      <div class="pay-detail">
        ...
        <div class="pay-cell">
          <span>配送费用：</span>
          <span v-if="!addressList[0]">请先选择配送地址</span>
...
export default {
  ...
  methods: {
    ...
    async getOrder() {
      const res = await checkOrder(this.mode, this.obj);
      this.order = res.data.order;
      this.personal = res.data.personal;
    },
    async submit() {
      const res = await submitOrder(this.mode, this.obj, this.remark);
      // console.log(res);
      Toast("支付成功");
