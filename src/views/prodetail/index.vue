  ...
  <div class="prodetail">
    ...
    >
      <div class="product">
        ...
        <div class="num-box">
          <span>数量</span>
          <van-stepper v-model="goodsCount" integer />
        </div>
        <div class="showbtn" v-if="true">
          <div class="btn" v-if="mode === 'cart'" @click="addCartAgain">
            加入购物车
          </div>
          <div class="btn now" v-else @click="buyNowCart">立刻购买</div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async addCartAgain() {
      ...
      const res = await addCartAPI(this.$route.params.id, this.goodsCount, 0);
  ...
  async mounted() {
    ...
    const resComments = await getProComments(this.$route.params.id, 3);
